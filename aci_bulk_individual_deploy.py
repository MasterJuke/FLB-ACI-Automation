...
def get_validated_available_ports(session, apic_url, node_id):
    """
    Get available ports with full validation.
    ...
    try:
        ...
        # Now check for policy groups and EPG bindings for each port
        # This is more expensive, so we do it on the filtered list
        validated_ports = []
        
        for port in ports:
            # Check for policy group (port selector) on this port
            has_policy_group = False
        ...
        if data["node_ids"]:
            ...
            for node_id in sorted(data["node_ids"]):
                ...
                if found_profile:
                    ...
                    confirm = input(f"        Use this profile for node {node_id}? (yes/no): ").strip().lower()
...
def main():
    ...
    custom_file = input("Press Enter to use default, or enter filename: ").strip()
    ...
    for i, dep in enumerate(deployments, 1):
        ...
        if check_policy_group_exists(session, apic_url, policy_group_name):
            ...
            use_existing = input("  Use existing policy group? (yes/no): ").strip().lower()
